...
import net.minecraft.src.buildcraft.krapht.ClientProxy;
...
public abstract class PipeLogisticsChassi extends RoutedPipe implements ISimpleInventoryEventHandler, IInventoryProvider, ISendRoutedItem, IProvideItems{
	...
	public void updateEntity() {
		...
		if (switchOrientationOnTick){
			...
			if(!APIProxy.isClient(worldObj)) {
				nextOrientation();
				CoreProxy.sendToPlayers(container.getUpdatePacket(), xCoord, yCoord, zCoord, 128, mod_BuildCraftCore.instance);
			}
		}
		...
		if(!init) {
			init = true;
			if(APIProxy.isClient(worldObj)) {
				ClientProxy.sendPacket(new PacketCoordinates(NetworkConstants.REQUEST_PIPE_UPDATE, xCoord, yCoord, zCoord).getPacket());
			}
		}
	}

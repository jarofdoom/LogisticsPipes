		...
		switch(guibutton.id)
		{
			case 0:
				...
				if(APIProxy.isClient(mc.theWorld)) {
					ModLoader.getMinecraftInstance().getSendQueue().addToSendQueue(new PacketPipeInteger(NetworkConstants.ADVANCED_EXTRACTOR_MODULE_INCLUDED_SET, pipe.xCoord, pipe.yCoord, pipe.zCoord, (_advancedExtractor.areItemsIncluded() ? 1 : 0) + (slot * 10)).getPacket());
				}
				break;
			case 1:
				if(!_advancedExtractor.connectedToSidedInventory()) {
				...
				if(APIProxy.isClient(mc.theWorld)) {
					ModLoader.getMinecraftInstance().getSendQueue().addToSendQueue(new PacketPipeInteger(NetworkConstants.ADVANCED_EXTRACTOR_MODULE_SNEAKY_GUI, pipe.xCoord, pipe.yCoord, pipe.zCoord, slot).getPacket());
				} else {

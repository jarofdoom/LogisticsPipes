		...
		if (i == 1 || c == 'e') {
			if (prevGuiID != -1) {
				if(!APIProxy.isClient(mc.theWorld)) {
					...
					super.keyTyped(c,i);
					ModLoader.getMinecraftInstance().getSendQueue().addToSendQueue(new PacketPipeInteger(NetworkConstants.GUI_BACK_PACKET, pipe.xCoord, pipe.yCoord, pipe.zCoord, prevGuiID + 10000).getPacket());
				}
			} else {
				super.keyTyped(c, i);
			}

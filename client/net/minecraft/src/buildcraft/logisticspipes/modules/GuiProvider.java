...
public class GuiProvider extends GuiWithPreviousGuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 0){
			...
			ModLoader.getMinecraftInstance().getSendQueue().addToSendQueue(new PacketPipeInteger(NetworkConstants.PROVIDER_MODULE_CHANGE_INCLUDE, _pipe.xCoord, _pipe.yCoord, _pipe.zCoord, _slot).getPacket());
		} else if (guibutton.id  == 1){
			_provider.nextExtractionMode();
			ModLoader.getMinecraftInstance().getSendQueue().addToSendQueue(new PacketPipeInteger(NetworkConstants.PROVIDER_MODULE_NEXT_MODE, _pipe.xCoord, _pipe.yCoord, _pipe.zCoord, _slot).getPacket());
		}
